...
export default function ProjectPage() {
  ...
  const body = md.render(project.body);
    ...
    <PageWrapperWithNavAndFooter>
      <div className="flex-grow flex flex-col gap-y-6 w-full max-w-4xl mx-auto p-6">
        <div className="flex flex-col gap-y-2">
          ...
          <h2 className="text-xl text-brand-gray-300 italic">{`${addLeadingZeros(project.date.month)}/${project.date.year}`}</h2>
        </div>
        ...
        <div className="flex justify-center w-full relative">
          <picture>
            <source
              ...
              srcSet={`${project.coverImage}cover_lg.webp`}
              type="image/webp"
            />
            <source
              media="(min-width: 380px)"
              srcSet={`${project.coverImage}cover_md.webp`}
              type="image/webp"
            />
            <source
              srcSet={`${project.coverImage}cover_sm.webp`}
              type="image/webp"
            />

            <source
              media="(min-width: 580px)"
              srcSet={`${project.coverImage}cover_lg.png`}
              type="image/png"
            />
            <source
              media="(min-width: 380px)"
              srcSet={`${project.coverImage}cover_md.png`}
              type="image/png"
            />
            <source
              srcSet={`${project.coverImage}cover_sm.png`}
              type="image/png"
            />

            <img
              src={`${project.coverImage}cover_sm.png`}
              width="580"
              height="337"
              alt={`${project.title}`}
